...
class BonusCards(enum.Enum):
  ...
  POWER3_SHIPPING = enum.auto() # 3 power income and +1 shipping for Phase I and II.
  SPADE_COIN2 = enum.auto() # 1 spade special action + 2 coin income.
  CULT_COIN4 = enum.auto() # 1 cult track special action + 4 coin income.
  DWELLING_COIN2 = enum.auto() # 1 vp per dwelling in board at end, 2 coin income.
  TRADING_POST_WORKER = enum.auto() # 2 vp per TP on board at end, 1 worker income.
...
class ScoringTile(enum.Enum):
  ...
  DWELLING_WATER4_PRIEST = enum.auto() # 2 VP per Dwelling built, and 1 priest per 4 Water.
  TP_WATER4_SPADE = enum.auto() # 2 VP per TP built, and get a spade for 4 Water. 
  TOWN_EARTH4_SPADE = enum.auto() # 5 VP per town built, and get 1 spade per 4 on cult track.
  STRONGHOLD_AIR2_WORKER = enum.auto() # 4 VP per stronghold, and get 1 worker per 2 air.
  SPADE_EARTH_COIN = enum.auto() # 2 VP per spade used, and get 1 coint per 1 earth.
  DWELLING_FIRE4_POWER4 = enum.auto() # 2 VP per dwelling built, and get 4 power per 4 fire.
  STRONGHOLD_FIRE2_WORKER = enum.auto() # 5 VP per stornghold, and get 1 worker per 2 fire.
...
class FavorTile(enum.Enum):
  ...
  TP1234_AIR = enum.auto() # Get 2/3/3/4 VP per 1/2/3/4 TPs when passing. 1 Air. 
  TOWN_FIRE2 = enum.auto() # Town founding only requires combined power of 6, instead of 7. 2 Fire.
  CULTACTION_AIR2 = enum.auto() # Special action to use cult track, plus 2 water. 
